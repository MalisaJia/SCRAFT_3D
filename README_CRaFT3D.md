...
| `train_diffusion.py` | TripoSG LoRA 微调训练循环（纯 latent，训练步内不渲染；可选 Learned Semantic Reward 逐样本损失加权） | `LoRALinear`、`LoRAEMA`、`LatentCacheDataset`、`inject_lora`、`lora_state_dict`、`load_lora_state_dict`、`build_dit_with_lora`、`forward_dit`、`train_step`、`parse_eval_num_steps`、`resolve_flow_shift`、`flow_match_sigmas`、`sample_latent_flow`、`decode_latent_to_mesh`、`run_evaluation`、`save_checkpoint`、`load_checkpoint`、`main` |
